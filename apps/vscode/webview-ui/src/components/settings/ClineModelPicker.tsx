import { CLAUDE_SONNET_1M_SUFFIX, type ModelInfo, openRouterDefaultModelId } from "@shared/api"
...
const ClineModelPicker: React.FC<ClineModelPickerProps> = ({
	...
	const resolveSelectedModelId = useCallback(
		(modelId?: string) => {
	...
	const [searchTerm, setSearchTerm] = useState(resolveSelectedModelId(modeFields.clineModelId))
	...
	useEffect(() => {
		...
		const currentModelId = resolveSelectedModelId(modeFields.clineModelId)
		setActiveTab(freeClineModelIdSet.has(normalizeModelId(currentModelId)) ? "free" : "recommended")
	}, [freeClineModelIdSet, initialTab, modeFields.clineModelId, resolveSelectedModelId])
	...
	}, [modeFields.clineModelId, resolveSelectedModelId])
